...
def estimate_total_examples(paths):
    """Estimate total number of examples by reading pre-calculated counts from tracking file.

    For datasets that split episodes into subskills, this reads the total from the
    processed_episodes.json tracking file. Falls back to counting from task info if needed.
    """
    import json
    from pathlib import Path

    # Try to load pre-calculated total from tracking file
    ...
    if tracking_file.exists():
        try:
            ...
            # Check if we have per-episode subskill counts
            episode_subskill_counts = tracking_data.get("episode_subskill_counts", {})

            if episode_subskill_counts:
                # Calculate total for the given paths
                total = 0
                for path in paths:
                    ...
                    total += episode_subskill_counts.get(episode_key, 1)

                if total > 0:
                    return total

            # Fallback: use total_subskills if available
            total_subskills = tracking_data.get("total_subskills")
            if total_subskills is not None and total_subskills > 0:
                return total_subskills
        except Exception as e:
            print(f"Warning: Could not read tracking file: {e}")

    # Fallback: count from task info files (original method)
    total = 0
    for path in paths:
        try:
            episode_path = Path(path)
            ...
            if task_json.exists():
                ...
                # Find this episode's action_configs
                for entry in task_data:
                    if str(entry.get("episode_id")) == episode_id:
                        action_configs = entry.get("label_info", {}).get("action_config", [])
                        total += len(action_configs) if action_configs else 1
                        break
                else:
                    # Episode not found in task info, assume 1 example
                    total += 1
            else:
                # No task info, assume 1 example per path
                total += 1
        except Exception:
            # If anything fails, assume 1 example per path
            total += 1

    return total if total > 0 else len(paths)


def parse_examples_from_generator(
    paths, fcn, split_name, total_num_examples, features, serializer
):
    # If total_num_examples is None, try to estimate it
    if total_num_examples is None:
        total_num_examples = estimate_total_examples(paths)
...
class ParallelSplitBuilder(split_builder_lib.SplitBuilder):
    ...
    def _build_from_generator(
        ...
        if self._n_workers <= 1:
            print("Generating sequentially (no multiprocessing)...")
            results = [
